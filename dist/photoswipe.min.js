...
if(/iP(hone|od)/.test(navigator.platform)){var g=navigator.appVersion.match(/OS (\d+)_(\d+)_?(\d+)?/);g&&g.length>0&&(g=parseInt(g[1],10),g>=1&&g<8&&(d.isOldIOSPhone=!0))}/iP(hone|od|ad)/.test(navigator.platform)&&(d.isIOS=!0);
...
// validate index
(isNaN(m)||m<0||m>=_b())&&(m=0),f.currItem=$b(m),(N.isIOS||N.isOldAndroid)&&(ua=!1),a.setAttribute("aria-hidden","false"),i.modal&&(ua?a.style.position="fixed":(a.style.position="absolute",a.style.top=e.getScrollY()+"px")),void 0===M&&(Ca("initialLayout"),M=K=e.getScrollY());
...
f.setContent(y[1],m),f.updateCurrItem(),Ca("afterInit"),(N.isOldIOSPhone||N.isOldAndroid)&&(
...
ta=0,f.updateCurrZoomItem(),z=m,Ca("afterChange")}}},updateSize:function(b){if(!ua&&i.modal&&(!N.isIOS||N.isOldIOSPhone)){var c=e.getScrollY();if(M!==c&&(a.style.top=c+"px",M=c),!b&&wa.x===window.innerWidth&&wa.y===window.innerHeight)return;wa.x=window.innerWidth,wa.y=window.innerHeight,
